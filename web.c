...
"    <button class=\"close\" onclick=\"closeDrawer()\" title=\"Close\">X</button>\n"
"  </div>\n"
"  <div id=\"drawer-body\">\n"
...
"    const name = n.name || '<span class=muted>?</span>';\n"
...
"    th.querySelector('.arr').textContent = (k===nodesSortKey) ? (nodesSortDir<0?'v':'^') : '';\n"
"  }\n"
"}\n"
...
"  const k = srcId < dstId ? srcId+'|'+dstId : dstId+'|'+srcId;\n"
"  let e = topoEdges[k];\n"
"  if (!e) e = topoEdges[k] = {a: srcId<dstId?srcId:dstId, b: srcId<dstId?dstId:srcId, snr: snr, count: 0};\n"
...
"  // Seed the node at a random spot near the canvas center so the layout\n"
"  // converges instead of starting all at (0,0).\n"
...
"    topoCtx.strokeStyle = topoSnrColor(e.snr, isH ? 0.95 : 0.45);\n"
...
"topoCanvas.addEventListener('click', ()=>{ if (topoHover) openDrawer(topoHover); });\n"
